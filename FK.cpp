    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // and the "euler2Rotation" function.
    for (int i = 0; i < localTransforms.size(); i++) {
        double jointR[9];
        euler2Rotation(eulerAngles[i], jointR, rotateOrders[i]);
        double orientationR[9];
        euler2Rotation(jointOrientationEulerAngles[i], orientationR, rotateOrders[i]); 
        Mat3d transformedR = Mat3d(orientationR) * Mat3d(jointR);
        localTransforms[i] = RigidTransform4d(transformedR, translations[i]);
  }
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
    for (int i = 0; i < jointUpdateOrder.size(); i++) {
        int curr = jointUpdateOrder[i];
        // root joint
        if (jointParents[curr] == -1) globalTransforms[curr] = localTransforms[curr];
        // child joint: globalTransform = parentGlobalTransform * localTransform
        else globalTransforms[curr] = globalTransforms[jointParents[curr]] * localTransforms[curr];
    }
