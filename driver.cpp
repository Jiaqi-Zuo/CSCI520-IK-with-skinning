...
static void idleFunction()
{
  ...
  char s[20] = "./imgs/xxxx.ppm";
  int i;
  // save screen to file
  s[7] = 48 + (sprite / 1000);
  s[8] = 48 + (sprite % 1000) / 100;
  s[9] = 48 + (sprite % 100) / 10;
  s[10] = 48 + sprite % 10;

  if (saveScreenToFile == 1)
  {
      saveScreenshot(windowWidth, windowHeight, s);
      if (sprite < 200) saveScreenToFile = 1; // save only once, change this if you want continuos image generation (i.e. animation)
      else sprite = 0;
      sprite++;
  }

  if (sprite >= 200) // allow only 200 snapshots
  {
      exit(0);
  }
